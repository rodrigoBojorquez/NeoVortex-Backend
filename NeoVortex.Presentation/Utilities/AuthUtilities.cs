...
using NeoVortex.Domain.Constants;

namespace NeoVortex.Presentation.Utilities;
...
public class AuthUtilities : IAuthUtilities
{
    ...
    public bool HasSuperAccess()
    {
        ...
        var claim = user.Claims.FirstOrDefault(c => c.Type == NeoVortexConstants.PermissionsClaim);
        
        if (claim is null) return false;

        return claim.Value.Contains(NeoVortexConstants.SuperAccessPermission);
    }
    ...
    public object? ShowAccessLevel(string accessToken)
    {
        ...
        var claim = token.Claims.FirstOrDefault(c => c.Type == NeoVortexConstants.PermissionsClaim);
