...
namespace NeoVortex.Presentation.Common.Middlewares;
...
public class PermissionAuthorizationMiddleware
{
    ...
    public async Task InvokeAsync(HttpContext context)
    {
        ...
        var permissionsClaim = user.FindFirst(c => c.Type == NeoVortexConstants.PermissionsClaim)?.Value;
        ...
        if (!permissions.Contains(requiredPermission) &&
            !permissions.Contains(NeoVortexConstants.SuperAccessPermission))
        {
            context.Response.StatusCode = StatusCodes.Status403Forbidden;
            return;
        }
